...
try {
  ...
  const targetPhrase = '이 타이 토코 오 사와라 나이데';
  const replacementPhrase = '이타이 토코 오 사와라 나이데';
  ...
  if (data && Array.isArray(data.translatedLines)) {
    ...
    if (changesMade > 0) {
      ...
      console.log(`\nSuccessfully updated ${filePath} with ${changesMade} changes related to reverting '${targetPhrase}'.`);
    } else {
      console.log(`\nNo changes needed for ${filePath} regarding reverting '${targetPhrase}'.`);
    }
  } else {
