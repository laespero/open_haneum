...
data.contextText.forEach((item, contextIndex) => {
  if (item.T0) {
    itemsWithT0.push({
      t0: item.T0,
...
console.log(`총 ${itemsWithT0.length}개의 T0 데이터를 찾았습니다.\n`);

let successCount = 0;
let failureCount = 0;
const failures = [];

// 각 T0에 대해 매칭 테스트 (전체 데이터)
const testItems = itemsWithT0; // 전체 데이터 테스트

testItems.forEach((item, index) => {
  const t0 = item.t0;
  const contextIndex = item.contextIndex;
  
  if (!data.rubyData || !data.rubyData[contextIndex]) {
    // console.log(`⚠️  ${index + 1}. T0 "${t0}"에 대응하는 rubyData를 찾을 수 없습니다. (contextIndex: ${contextIndex})`);
    failureCount++;
    failures.push({ index: index + 1, t0, reason: 'rubyData 없음' });
    return;
  }
  ...
  // T0 문장을 HTML에서 찾기 시도
  const foundRuby = findRuby(htmlStr, t0);
  
  if (foundRuby) {
    // console.log(`✅ ${index + 1}. "${t0}" → 매칭 성공`);
    successCount++;
  } else {
    console.log(`❌ ${index + 1}. "${t0}" → 매칭 실패`);
    console.log(`   전체 HTML: ${htmlStr}`);
    console.log(`   T0 길이: ${t0.length}, HTML plainText 추출 결과 확인 필요\n`);
    failureCount++;
    failures.push({ index: index + 1, t0, html: htmlStr, reason: '매칭 실패' });
  }
});

// 결과 요약
console.log('='.repeat(80));
console.log(`T0 매칭 테스트 결과 (전체 ${itemsWithT0.length}개 테스트):`);
...
if (failures.length > 0) {
  ...
  failures.forEach(failure => {
    console.log(`  ${failure.index}. "${failure.t0}" (${failure.reason})`);
  });
} 