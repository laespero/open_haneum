const fs = require('fs');
const path = require('path');

const filePath = path.join(__dirname, '..', 'songs', 'deep_no_regret.json');

try {
  let rawData = fs.readFileSync(filePath, 'utf-8');
  let data = JSON.parse(rawData);
  let changesMade = 0;

  // 정규식: 한글 음절 다음에 '나이'가 붙어있는 경우를 찾습니다.
  // (주의: '데'나 '카라' 등이 뒤에 붙는 경우도 고려해야 할 수 있으나, 우선은 '나이'로 끝나는 경우만 처리)
  // 좀 더 정교하게 하려면 '나이다' '나이데' '나이카라' 등도 별도 처리하거나 정규식을 확장해야 합니다.
  const regex = /([가-힣]+)(나이)/g;
  const replacementPattern = '$1 $2';

  if (data && Array.isArray(data.translatedLines)) {
    data.translatedLines.forEach(line => {
      const fieldsToUpdate = ['R0', 'R1', 'R2', 'XR'];
      fieldsToUpdate.forEach(field => {
        if (line[field] && typeof line[field] === 'string' && line[field].match(regex)) {
          const originalValue = line[field];
          // 이미 띄어쓰기가 되어 있는 경우는 제외 (예: "이라 나이"는 변경하지 않음)
          // 그리고 '나이' 자체가 단독으로 쓰이는 경우 (예: '나이')는 건드리지 않도록, 앞에 반드시 한글 음절이 있는 경우만 대상
          if (!originalValue.match(/([가-힣]+)\s+(나이)/g) && originalValue.match(regex)) {
            line[field] = originalValue.replace(regex, replacementPattern);
            // '나이데' 같은 경우 '나이 데'가 아닌 '나이 데'가 되어야 하므로, 후처리로 공백 정리
            line[field] = line[field].replace(/\s+데/g, ' 데').replace(/\s+카라/g, ' 카라').replace(/\s+케도/g, ' 케도');

            if (originalValue !== line[field]) {
              console.log(`- ${field}: "${originalValue}" -> "${line[field]}" (T0: "${line.T0}")`);
              changesMade++;
            }
          }
        }
      });

      if (line.LI && Array.isArray(line.LI)) {
        line.LI.forEach(item => {
          fieldsToUpdate.forEach(field => {
            if (item[field] && typeof item[field] === 'string' && item[field].match(regex)) {
              const originalValue = item[field];
              if (!originalValue.match(/([가-힣]+)\s+(나이)/g) && originalValue.match(regex)) {
                 item[field] = originalValue.replace(regex, replacementPattern);
                 item[field] = item[field].replace(/\s+데/g, ' 데').replace(/\s+카라/g, ' 카라').replace(/\s+케도/g, ' 케도');

                 if (originalValue !== item[field]) {
                    let context = `(T1: "${item.T1}", T0: "${line.T0}")`;
                    if (field === 'R2') context = `(T2: "${item.T2}", ${context})`;
                    if (field === 'XR') context = `(XE: "${item.XE}", ${context})`;
                    console.log(`  - ${field}: "${originalValue}" -> "${item[field]}" ${context}`);
                    changesMade++;
                 }
              }
            }
          });
        });
      }
    });

    if (changesMade > 0) {
      fs.writeFileSync(filePath, JSON.stringify(data, null, 2), 'utf-8');
      console.log(`\nSuccessfully updated ${filePath} with ${changesMade} changes related to '나이' spacing.`);
    } else {
      console.log(`\nNo changes needed for ${filePath} regarding '나이' spacing.`);
    }
  } else {
    console.error('Error: Could not find translatedLines array or data structure is not as expected.');
  }
} catch (error) {
  console.error('Error processing the file:', error);
} 