...
try {
  ...
  // 정규식: 한글 음절 다음에 '나사이'가 붙어있는 경우를 찾습니다.
  const regex = /([가-힣]+)(나사이)/g;
  ...
  if (data && Array.isArray(data.translatedLines)) {
    data.translatedLines.forEach(line => {
      ...
      fieldsToUpdate.forEach(field => {
        if (line[field] && typeof line[field] === 'string' && line[field].match(regex)) {
          ...
          if (!originalValue.match(/([가-힣]+)\s+(나사이)/g) && originalValue.match(regex)) {
    ...
    if (changesMade > 0) {
      ...
      console.log(`\nSuccessfully updated ${filePath} with ${changesMade} changes related to '나사이' spacing.`);
    } else {
      console.log(`\nNo changes needed for ${filePath} regarding '나사이' spacing.`);
    }
  } else {
